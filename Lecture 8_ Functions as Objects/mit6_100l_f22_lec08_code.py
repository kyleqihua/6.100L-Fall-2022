...
def h(y):
    ...
    # b = x + 1 
    # print(a + b)
    # can run, use x as global variable, 
    # the above one and the below one can run individually, but cannot co-exit 
    # otherwise, showing local variable 'x' referenced before assignment 
    # x = 7
    # x = x + 1
    # print(f"this is new x inside h function scope: {x}")
    # can run, use x as local variable, and assign it first 
    # b = x + 1
    # x = b
    # print(x) 
    # not run, showing local variable 'x' referenced before assignment
    x = x + 1 #leads to an error without line `global x` inside h
    # not run, showing local variable 'x' referenced before assignment
    # from the course, teaching you cannot modify the global variable inside a function?
    # z = x + 1
    # the lectuer is wrong here when she said you will get the same error using z 
